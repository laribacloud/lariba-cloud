...
# =========================================================
# Dependency: Authenticate via X-API-Key
# =========================================================
...
def create_api_key(payload: CreateApiKeyRequest, db: Session = Depends(get_db)):
    ...
    return CreateApiKeyResponse(
        ...
        api_key=plaintext_key,  # returned ONCE
        expires_at=api_key.expires_at.isoformat() if api_key.expires_at else None,
    )


@router.get("", response_model=List[ApiKeyListItem])
def list_api_keys(project_id: UUID, db: Session = Depends(get_db)):
...
def revoke_api_key(api_key_id: UUID, db: Session = Depends(get_db)):
